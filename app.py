...
def state_cache_worker():
    ...
    while STATE_CACHE_RUNNING:
        ...
        time.sleep(max(0.10, 0.75-elapsed))
...
def build_state():
    ...
    return clean({
        "status":"ok","version":"V2.20 / V10.34",
        "server_time":now,
        "market":{
...
app = FastAPI(title="BTC15M V2.20 / V10.34 Ghost + L2 + Predictive Decision", lifespan=lifespan)
...
def state():
    ...
    return JSONResponse({"status":"starting","ok":False,"error":cache_error or "state_cache_warming"}, headers={"Cache-Control":"no-store"})

@app.get("/api/health")
def health():
    # V2.20: health is cache-only; never perform exchange/network/model work here.
    with STATE_LOCK:
        age=(time.time()-STATE_CACHE_AT) if STATE_CACHE_AT else None
        ok=STATE_CACHE is not None and (age is None or age < 15.0)
        err=STATE_CACHE_ERROR
    return JSONResponse({"ok":ok,"version":"V2.20 / V10.34","cache_age_s":age,"cache_error":err}, status_code=200 if ok else 503, headers={"Cache-Control":"no-store"})
