...
def _capture_final_target_second(r, now):
    ...
    V2.28.38 repairs the old snapshot-only collector.  Instead of capturing only
...
app = FastAPI(title="BTC15M V2.28.38 / V10.39.22 Server Auto Paper V1 + Atomic RDFZ + Fast Rollover", lifespan=lifespan)
...
def prospective_instrumentation_tick(d):
    ...
    if now-_LAST_PROSPECTIVE_AT >= PROSPECTIVE_MIN_INTERVAL_S:
        ...
        rec={
            ...
            "app_version":"V2.28.38","research_version":"V10.39.22",
    ...
    if now-_LAST_WS_HEALTH_AT >= WS_HEALTH_INTERVAL_S:
        ...
        _append_ndjson(WS_HEALTH_LOG_PATH,WS_HEALTH_LOG_LOCK,{
            "schema":"BTC15M_WS_HEALTH_V1","app_version":"V2.28.38",
...
def _rdfz_manifest(files=None):
    ...
    return {"schema":"RDFZ_V2","app_version":"V2.28.38","research_version":"V10.39.22",
...
def _rdfz_write_export(mode="compact"):
    ...
    try:
        ...
        filename=f"BTC15M_WEB_RDFZ_V2.28.38_{mode.upper()}_{stamp_start}-{stamp_end}.zip"
        fd,tmp_name=tempfile.mkstemp(prefix=f"btc15m_rdfz_{mode}_",suffix=".zip")
        os.close(fd); archive=Path(tmp_name)
        manifest={"schema":"RDFZ_EXPORT_V7_SAFE_COMPRESSED","app_version":"V2.28.38","research_version":"V10.39.22",
