...
def build_state():
    ...
    if candles is not None and len(candles):
        ...
        if not candle_data:
            c = candles.tail(90)
            spark = [{"t":clean(t), "v":clean(v)} for t, v in zip(c["time"], c["close"])]
            candle_data = [{"t":clean(row["time"]),"o":clean(row["open"]),"h":clean(row["high"]),"l":clean(row["low"]),"c":clean(row["close"])} for _,row in c.iterrows() if all(k in row for k in ["time","open","high","low","close"])]
    with MX_LOCK:
    ...
    return clean({
        "status":"ok","version":"V2.16 / V10.31",
        "server_time":now,
        "market":{
...
app = FastAPI(title="BTC15M V2.16 / V10.31 Profit Protection + Reliable Rollover", lifespan=lifespan)
