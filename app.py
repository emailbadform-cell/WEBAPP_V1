...
def prospective_instrumentation_tick(d):
    ...
    if now-_LAST_PROSPECTIVE_AT >= PROSPECTIVE_MIN_INTERVAL_S:
        ...
        rec={
            ...
            "app_version":"V2.28.41","research_version":"V10.39.24",
    ...
    if now-_LAST_WS_HEALTH_AT >= WS_HEALTH_INTERVAL_S:
        ...
        _append_ndjson(WS_HEALTH_LOG_PATH,WS_HEALTH_LOG_LOCK,{
            "schema":"BTC15M_WS_HEALTH_V1","app_version":"V2.28.41",
...
def runtime_health():
    ...
    return JSONResponse({"ok":True,"version":"V2.28.41","uptime_cache_age_s":(time.time()-STATE_CACHE_AT) if STATE_CACHE_AT else None,
...
def _rdfz_manifest(files=None):
    ...
    return {"schema":"RDFZ_V2","app_version":"V2.28.41","research_version":"V10.39.24",
...
def _rdfz_write_export(mode="compact"):
    ...
    try:
        ...
        filename=f"BTC15M_WEB_RDFZ_V2.28.41_{mode.upper()}_{stamp_start}-{stamp_end}.zip"
        fd,tmp_name=tempfile.mkstemp(prefix=f"btc15m_rdfz_{mode}_",suffix=".zip")
        os.close(fd); archive=Path(tmp_name)
        manifest={"schema":"RDFZ_EXPORT_V7_SAFE_COMPRESSED","app_version":"V2.28.41","research_version":"V10.39.24",
