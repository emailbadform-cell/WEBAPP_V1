...
def build_state():
    ...
    return clean({
        "status":"ok","version":"V2.28.31 / V10.39.18",
        "server_time":now,
        "market":{
        ...
        "trend_transition": core.trend_transition_challenger_v1(r, getattr(engine,"ticks",None), now),
            "move_projection":mp,
        "mp_of_context":mp_of,
        "of_early":early,
        "of_micro":of_micro,
...
def prospective_instrumentation_tick(d):
    ...
    if now-_LAST_PROSPECTIVE_AT >= PROSPECTIVE_MIN_INTERVAL_S:
        ...
        rec={
            ...
            "app_version":"V2.28.31","research_version":"V10.39.18",
...
def _rdfz_manifest(files=None):
    ...
    return {"schema":"RDFZ_V2","app_version":"V2.28.31","research_version":"V10.39.18",
...
def rdfz_download():
    ...
    batch_id=f"WEB_V2.28.31_{stamp_start}-{stamp_end}_{now.strftime('%f')}"
    ...
    filename=f"BTC15M_WEB_RDFZ_V2.28.31_{stamp_start}-{stamp_end}.zip"
...
def instrumentation_status():
    ...
    return JSONResponse({"ok":True,"version":"V2.28.31 / V10.39.18","rdfz_dir":str(RDFZ_DIR),
