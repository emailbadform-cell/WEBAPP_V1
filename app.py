...
def _capture_final_target_second(r, now):
    ...
    val=(r or {}).get("btc")
    if not ticker or pd.isna(closep) or pd.isna(qt) or val is None: return {}
    start=closep-pd.Timedelta(seconds=60)
    if qt < start or qt > closep: return {}
    sec=qt.floor("s").isoformat()
    with FINAL_TARGET_LOCK:
        # Drop stale contracts but never delete a captured second from the active contract.
        for k in list(FINAL_TARGET_SECONDS):
            if k != ticker: FINAL_TARGET_SECONDS.pop(k,None)
        bucket=FINAL_TARGET_SECONDS.setdefault(ticker,{})
        bucket[sec]=float(val)  # LAST_TICK_PER_SOURCE_SECOND
        return dict(bucket)
...
def build_state():
    ...
    final_target = core.settlement_window_state(engine.ticks, r.get('btc'), r.get('target'), r.get('close_time'), now, preserved_ft_seconds)
...
app = FastAPI(title="BTC15M V2.28.37 / V10.39.21 Server Auto Paper V1 + Atomic RDFZ + Fast Rollover", lifespan=lifespan)
...
PROSPECTIVE_LOG_PATH = Path(os.getenv("PROSPECTIVE_LOG_PATH", str(RDFZ_DIR / "BTC15M_V10_39_21_PROSPECTIVE_ARCHITECT_SNAPSHOTS.ndjson")))
...
def prospective_instrumentation_tick(d):
    ...
    if now-_LAST_PROSPECTIVE_AT >= PROSPECTIVE_MIN_INTERVAL_S:
        ...
        rec={
            ...
            "app_version":"V2.28.37","research_version":"V10.39.21",
    ...
    if now-_LAST_WS_HEALTH_AT >= WS_HEALTH_INTERVAL_S:
        ...
        _append_ndjson(WS_HEALTH_LOG_PATH,WS_HEALTH_LOG_LOCK,{
            "schema":"BTC15M_WS_HEALTH_V1","app_version":"V2.28.37",
...
def auto_paper_tick(d):
    ...
    with AUTO_PAPER_LOCK:
        ...
        # Settle stale position on rollover using last observed BRTI/target proxy, preserving V1 semantics.
        if ticker!=pos.get("ticker") or float(m.get("seconds_left") or 0)<=0:
            brti=m.get("brti") if ticker==pos.get("ticker") else pos.get("last_brti"); target=m.get("target") if ticker==pos.get("ticker") else pos.get("target")
            if brti is not None and target is not None:
                ...
                _auto_event("EXIT",d,{"exit":xp,"pnl":pnl,"reason":"EXPIRY / ROLLOVER","result_type":"PROXY_NOT_OFFICIAL","peak_bid":pos["peak_bid"],"trough_bid":pos["trough_bid"]},pos); AUTO_PAPER["last_exit"]={"trade_id":pos["trade_id"],"pnl":pnl,"reason":"EXPIRY / ROLLOVER","at":now}; AUTO_PAPER["position"]=None; _save_auto_state()
            return
...
def _rdfz_manifest(files=None):
    ...
    return {"schema":"RDFZ_V2","app_version":"V2.28.37","research_version":"V10.39.21",
...
_RDFZ_EXPORT_LOCK = threading.Lock()
_RDFZ_COMPACT_NAMES = {
...
def _rdfz_is_compact_file(p: Path):
    # Architecture snapshots are versioned, so match by semantic suffix.
    return p.name in _RDFZ_COMPACT_NAMES or "PROSPECTIVE_ARCHITECT_SNAPSHOTS" in p.name

def _rdfz_capture_export_specs(files):
    """Capture a point-in-time export safely.

    Append-only NDJSON uses a frozen byte boundary. Mutable JSON state is copied
    into memory immediately so an atomic rewrite cannot truncate the export.
    """
    specs=[]
    for p in files:
        try:
            if p.suffix.lower()==".json":
                data=p.read_bytes()
                specs.append({"path":p,"size":len(data),"mtime":p.stat().st_mtime,"bytes":data,"mutable_snapshot":True})
            else:
                st=p.stat()
                specs.append({"path":p,"size":int(st.st_size),"mtime":float(st.st_mtime),"mutable_snapshot":False})
        except (FileNotFoundError,OSError):
            continue
    return specs

def _rdfz_write_export(mode="compact"):
    if mode not in ("compact","full"):
        raise ValueError("invalid RDFZ export mode")
    if not _RDFZ_EXPORT_LOCK.acquire(blocking=False):
        return None, JSONResponse({"ok":False,"error":"rdfz_export_already_running"},status_code=409,headers={"Cache-Control":"no-store"})
    archive=None
    try:
        RDFZ_DIR.mkdir(parents=True,exist_ok=True)
        active=list(_rdfz_active_files())
        selected=active if mode=="full" else [p for p in active if _rdfz_is_compact_file(p)]
        specs=_rdfz_capture_export_specs(selected)
        now=datetime.now(timezone.utc)
        # Stats only scan frozen NDJSON boundaries. Mutable JSON has no event rows.
        stat_specs=[x for x in specs if not x.get("mutable_snapshot")]
        rows,first,last=_rdfz_stats(specs=stat_specs)
        row_by_name={r.get("name") or r.get("file") or r.get("path"):r for r in rows}
        # Preserve manifest visibility for mutable snapshots without pretending they are NDJSON.
        for x in specs:
            if x.get("mutable_snapshot"):
                rows.append({"name":x["path"].name,"bytes":x["size"],"rows":None,"snapshot":"immutable_in_memory"})
        stamp_start=(first or now).strftime("%Y%m%d_%H%M%S")
        stamp_end=(last or now).strftime("%Y%m%d_%H%M%S")
        filename=f"BTC15M_WEB_RDFZ_V2.28.37_{mode.upper()}_{stamp_start}-{stamp_end}.zip"
        fd,tmp_name=tempfile.mkstemp(prefix=f"btc15m_rdfz_{mode}_",suffix=".zip")
        os.close(fd); archive=Path(tmp_name)
        manifest={"schema":"RDFZ_EXPORT_V7_SAFE_COMPRESSED","app_version":"V2.28.37","research_version":"V10.39.21",
                  "generated_at_utc":now.isoformat(),"data_start_utc":(first.isoformat() if first else None),
                  "data_end_utc":(last.isoformat() if last else None),"compression":"ZIP_DEFLATED_LEVEL_1",
                  "export_mode":mode,"source_files_preserved":True,
                  "snapshot_boundary":"frozen_ndjson_bytes_plus_in_memory_mutable_json",
                  "file_count":len(rows),"total_rows":sum((r.get("rows") or 0) for r in rows),"files":rows}
        t0=time.perf_counter(); source_bytes=sum(x["size"] for x in specs)
        with zipfile.ZipFile(archive,"w",compression=zipfile.ZIP_DEFLATED,compresslevel=1,allowZip64=True) as z:
            z.writestr("RDFZ/RDFZ_MANIFEST.json",json.dumps(manifest,indent=2))
            for spec in specs:
                fp=spec["path"]; limit=spec["size"]
                arc=f"RDFZ/{fp.relative_to(RDFZ_DIR).as_posix()}"
                if spec.get("mutable_snapshot"):
                    z.writestr(arc,spec["bytes"])
                    continue
                zi=zipfile.ZipInfo(arc,date_time=datetime.now().timetuple()[:6]); zi.compress_type=zipfile.ZIP_DEFLATED
                remaining=limit
                with fp.open("rb") as src, z.open(zi,"w",force_zip64=(limit>=2_000_000_000)) as dst:
                    while remaining>0:
                        chunk=src.read(min(1024*1024,remaining))
                        if not chunk:
                            raise RuntimeError(f"append-only RDFZ source unexpectedly truncated during export: {fp.name}")
                        dst.write(chunk); remaining-=len(chunk)
        with zipfile.ZipFile(archive,"r") as verify_zip:
            bad=verify_zip.testzip()
            if bad: raise RuntimeError(f"RDFZ ZIP verification failed: {bad}")
            for spec in specs:
                arc=f"RDFZ/{spec['path'].relative_to(RDFZ_DIR).as_posix()}"
                if verify_zip.getinfo(arc).file_size != spec["size"]:
                    raise RuntimeError(f"RDFZ snapshot size mismatch: {arc}")
        info={"filename":filename,"created_utc":now.isoformat(),"data_start_utc":manifest["data_start_utc"],
              "data_end_utc":manifest["data_end_utc"],"source_bytes":source_bytes,"zip_bytes":archive.stat().st_size,
              "compression":manifest["compression"],"build_seconds":round(time.perf_counter()-t0,3),
              "export_mode":mode,"source_files_preserved":True}
        try:(RDFZ_DIR/"RDFZ_LAST_EXPORT.json").write_text(json.dumps(info,indent=2),encoding="utf-8")
        except Exception:pass
        return (archive,filename,info),None
    except Exception as exc:
        if archive:
            try: archive.unlink(missing_ok=True)
            except Exception: pass
        return None, JSONResponse({"ok":False,"error":"rdfz_export_failed","detail":str(exc)},status_code=500,headers={"Cache-Control":"no-store"})
    finally:
        _RDFZ_EXPORT_LOCK.release()

@app.get("/api/rdfz/download")
def rdfz_download():
    """Default compact, compressed research export for fast routine downloads."""
    result,error=_rdfz_write_export("compact")
    if error:return error
    archive,filename,info=result
    return FileResponse(str(archive),media_type="application/zip",filename=filename,
                        headers={"Cache-Control":"no-store","X-RDFZ-Export-Mode":"compact-safe-compressed"},
                        background=BackgroundTask(lambda: archive.unlink(missing_ok=True)))

@app.get("/api/rdfz/download-full")
def rdfz_download_full():
    """Optional full raw export including high-volume Kalshi WS events."""
    result,error=_rdfz_write_export("full")
    if error:return error
    archive,filename,info=result
    return FileResponse(str(archive),media_type="application/zip",filename=filename,
                        headers={"Cache-Control":"no-store","X-RDFZ-Export-Mode":"full-safe-compressed"},
                        background=BackgroundTask(lambda: archive.unlink(missing_ok=True)))

@app.get("/api/rdfz/exports")
def rdfz_exports():
    # V2.28.35 no longer stores server-side export copies.
    return JSONResponse({"ok":True,"exports":[],"mode":"cumulative_nondestructive","note":"Exports are streamed from active RDFZ data; no duplicate server archive is retained."},headers={"Cache-Control":"no-store"})

@app.get("/api/rdfz/export-file")
def rdfz_export_file(id: str):
    root=(RDFZ_DIR/"_exports").resolve()
    ...
    return FileResponse(str(target),media_type="application/zip",filename=target.name,
                        headers={"Cache-Control":"no-store","X-RDFZ-Export-Mode":"existing"})
...
async def rdfz_delete_export(request: Request):
    ...
    if not target.exists() or not target.is_file() or target.suffix.lower()!=".zip":
        return JSONResponse({"ok":False,"error":"export_not_found"},status_code=404)
    size=target.stat().st_size; parent=target.parent; target.unlink()
    # Clean empty batch directory only; never delete other exports.
    try:
        if parent != root and not any(parent.iterdir()): parent.rmdir()
    except Exception: pass
...
def rdfz_download_last():
    # V2.28.35 exports are intentionally not retained server-side. Generate a
    # fresh cumulative export instead; active source data is still present.
    return JSONResponse({"ok":False,"error":"server_export_cache_disabled","use":"/api/rdfz/download","mode":"cumulative_nondestructive"},status_code=409,headers={"Cache-Control":"no-store"})

@app.post("/api/rdfz/clear-exported")
def rdfz_clear_exported():
    return JSONResponse({"ok":True,"cleared_bytes":0,"mode":"cumulative_nondestructive","note":"No duplicate server-side export cache is created in V2.28.35."},headers={"Cache-Control":"no-store"})
