...
def prospective_instrumentation_tick(d):
    ...
    if now-_LAST_PROSPECTIVE_AT >= PROSPECTIVE_MIN_INTERVAL_S:
        ...
        rec={
            ...
            "app_version":"V2.28.35","research_version":"V10.39.19",
...
def _rdfz_manifest(files=None):
    ...
    return {"schema":"RDFZ_V2","app_version":"V2.28.35","research_version":"V10.39.19",
...
def rdfz_download():
    """V2.28.35 atomic-boundary cumulative, non-destructive export.

    Byte lengths are frozen before scanning. Manifest and ZIP are both built
    from those exact byte boundaries while active files remain appendable.
    """
    RDFZ_DIR.mkdir(parents=True,exist_ok=True)
    active=list(_rdfz_active_files())
    specs=_rdfz_snapshot_specs(active)
    now=datetime.now(timezone.utc)
    rows,first,last=_rdfz_stats(specs=specs)
    stamp_start=(first or now).strftime("%Y%m%d_%H%M%S")
    stamp_end=(last or now).strftime("%Y%m%d_%H%M%S")
    filename=f"BTC15M_WEB_RDFZ_V2.28.35_{stamp_start}-{stamp_end}.zip"
    fd,tmp_name=tempfile.mkstemp(prefix="btc15m_rdfz_",suffix=".zip")
    os.close(fd); archive=Path(tmp_name)
    manifest={"schema":"RDFZ_EXPORT_V6_ATOMIC_BOUNDARY_CUMULATIVE","app_version":"V2.28.35","research_version":"V10.39.19",
              "generated_at_utc":now.isoformat(),
              "data_start_utc":(first.isoformat() if first else None),"data_end_utc":(last.isoformat() if last else None),
              "compression":"ZIP_STORED","export_mode":"cumulative_nondestructive_atomic_boundary",
              "source_files_preserved":True,"snapshot_boundary":"byte_length_captured_before_manifest_scan",
              "file_count":len(rows),"total_rows":sum((r.get("rows") or 0) for r in rows),"files":rows}
    ...
    try:
            ...
            for spec in specs:
                ...
                with fp.open("rb") as src, z.open(zi,"w",force_zip64=(limit>=2_000_000_000)) as dst:
                    while remaining>0:
                        ...
                        if not chunk: raise RuntimeError(f"RDFZ source truncated during export: {fp.name}")
        ...
        with zipfile.ZipFile(archive,"r") as verify_zip:
            ...
            # Verify exact frozen byte counts inside archive.
            for spec in specs:
    ...
    info={"filename":filename,"created_utc":now.isoformat(),
          "data_start_utc":manifest["data_start_utc"],"data_end_utc":manifest["data_end_utc"],
          "source_bytes":source_bytes,"zip_bytes":archive.stat().st_size,"compression":"ZIP_STORED",
          "build_seconds":round(time.perf_counter()-t0,3),"export_mode":"cumulative_nondestructive_atomic_boundary",
          "source_files_preserved":True,"snapshot_boundary":"byte_length_captured_before_manifest_scan"}
    try:
    ...
    return FileResponse(str(archive),media_type="application/zip",filename=filename,
                        headers={"Cache-Control":"no-store","X-RDFZ-Export-Mode":"cumulative-nondestructive-atomic-boundary"},
