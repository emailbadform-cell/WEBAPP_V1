...
def build_state():
    ...
    return clean({
        "status":"ok","version":"V2.28.21 / V10.39.11",
        "server_time":now,
        "market":{
...
# V2.28.21 / V10.39.11: instrumentation-only prospective recorder.
# No prediction or Auto Paper policy authority is changed by this layer.
PROSPECTIVE_LOG_PATH = Path(os.getenv("PROSPECTIVE_LOG_PATH", str(RDFZ_DIR / "BTC15M_V10_39_11_PROSPECTIVE_ARCHITECT_SNAPSHOTS.ndjson")))
...
def prospective_instrumentation_tick(d):
    ...
    if now-_LAST_PROSPECTIVE_AT >= PROSPECTIVE_MIN_INTERVAL_S:
        ...
        rec={
            ...
            "app_version":"V2.28.21","research_version":"V10.39.11",
    ...
    if now-_LAST_WS_HEALTH_AT >= WS_HEALTH_INTERVAL_S:
        ...
        _append_ndjson(WS_HEALTH_LOG_PATH,WS_HEALTH_LOG_LOCK,{
            "schema":"BTC15M_WS_HEALTH_V1","app_version":"V2.28.21",
...
def _rdfz_manifest():
    ...
    return {"schema":"RDFZ_V1","app_version":"V2.28.21","research_version":"V10.39.11",
...
def instrumentation_status():
    ...
    return JSONResponse({"ok":True,"version":"V2.28.21 / V10.39.11","rdfz_dir":str(RDFZ_DIR),
