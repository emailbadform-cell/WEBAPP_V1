...
def build_state():
    ...
    final_target = core.settlement_window_state(engine.ticks, r.get('btc'), r.get('target'), r.get('close_time'), now)
    ...
    return clean({
        "status":"ok","version":"V2.28.27 / V10.39.14",
        "server_time":now,
        "market":{
...
def prospective_instrumentation_tick(d):
    ...
    if now-_LAST_PROSPECTIVE_AT >= PROSPECTIVE_MIN_INTERVAL_S:
        ...
        rec={
            ...
            "app_version":"V2.28.28","research_version":"V10.39.16",
...
AUTO_PAPER={"enabled":False,"position":None,"decision_side":None,"decision_since":0.0,"last_shadow_key":None,"last_exit":None}
...
def _load_auto_state():
    try:
        if AUTO_PAPER_STATE_PATH.exists():
            ...
            AUTO_PAPER["enabled"]=bool(j.get("enabled",False)); AUTO_PAPER["position"]=j.get("position"); AUTO_PAPER["last_exit"]=j.get("last_exit")
    except Exception: pass
...
def auto_paper_tick(d):
    ...
    with AUTO_PAPER_LOCK:
        ...
        if pos is None and g["pass"]:
            bid=g.get("bid"); entry=float(g["ask"]); pos={"trade_id":str(uuid.uuid4()),"ticker":ticker,"side":side,"stake":100.0,"entry":entry,"quantity":100.0/entry,"peak_bid":float(bid) if bid is not None else entry,"trough_bid":float(bid) if bid is not None else entry,"entered_at":now,"opp_since":0.0,"last_brti":m.get("brti"),"target":m.get("target")}
            pos["last_update_event_at"]=now
            AUTO_PAPER["position"]=pos; _save_auto_state(); _auto_event("ENTRY",d,{"entry_source":"SERVER_AUTO_V1","gate":g},pos); return
...
def _rdfz_manifest(files=None):
    ...
    for p in files:
        try:
            rows.append({"name":p.relative_to(RDFZ_DIR).as_posix(),"bytes":p.stat().st_size,
                         "modified_utc":datetime.fromtimestamp(p.stat().st_mtime,timezone.utc).isoformat()})
        except Exception: pass
    first,last=_rdfz_range(files)
    return {"schema":"RDFZ_V2","app_version":"V2.28.28","research_version":"V10.39.16",
            "folder":str(RDFZ_DIR),"generated_at_utc":datetime.now(timezone.utc).isoformat(),
            "data_start_utc":first.isoformat() if first else None,"data_end_utc":last.isoformat() if last else None,"files":rows}
...
def rdfz_download():
    ...
    batch_id=f"WEB_V2.28.27_{stamp_start}-{stamp_end}_{now.strftime('%f')}"
    ...
    manifest=_rdfz_manifest([])
    manifest.update({"schema":"RDFZ_EXPORT_BATCH_V3_FAST","batch_id":batch_id,"data_start_utc":(first or now).isoformat(),
                     "data_end_utc":(last or now).isoformat(),"compression":"ZIP_STORED",
                     "files":[{"name":p.relative_to(batch).as_posix(),"bytes":p.stat().st_size} for p in moved]})
    (batch/"RDFZ_MANIFEST.json").write_text(json.dumps(manifest,indent=2),encoding="utf-8")
    filename=f"BTC15M_WEB_RDFZ_V2.28.27_{stamp_start}-{stamp_end}.zip"
...
def instrumentation_status():
    ...
    return JSONResponse({"ok":True,"version":"V2.28.27 / V10.39.14","rdfz_dir":str(RDFZ_DIR),
