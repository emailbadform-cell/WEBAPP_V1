...
def build_state():
    ...
    return clean({
        "status":"ok","version":"V2.28.24 / V10.39.14",
        "server_time":now,
        "market":{
...
def prospective_instrumentation_tick(d):
    ...
    if now-_LAST_PROSPECTIVE_AT >= PROSPECTIVE_MIN_INTERVAL_S:
        ...
        rec={
            ...
            "app_version":"V2.28.24","research_version":"V10.39.14",
...
def _rdfz_manifest(files=None):
    ...
    return {"schema":"RDFZ_V2","app_version":"V2.28.24","research_version":"V10.39.14",
...
def rdfz_download():
    # Rotate active files into a frozen batch first. Writers reopen by pathname,
    # so subsequent observations immediately create fresh active files and are
    # never removed by clearing this exported batch.
    RDFZ_DIR.mkdir(parents=True,exist_ok=True)
    ...
    batch_id=f"WEB_V2.28.24_{stamp_start}-{stamp_end}_{now.strftime('%f')}"
    batch=RDFZ_DIR/"_exports"/batch_id/"RDFZ"
    ...
    manifest=_rdfz_manifest([])
    manifest.update({"schema":"RDFZ_EXPORT_BATCH_V2","batch_id":batch_id,"data_start_utc":(first or now).isoformat(),
                     "data_end_utc":(last or now).isoformat(),"files":[{"name":p.relative_to(batch).as_posix(),"bytes":p.stat().st_size} for p in moved]})
    (batch/"RDFZ_MANIFEST.json").write_text(json.dumps(manifest,indent=2),encoding="utf-8")
    filename=f"BTC15M_WEB_RDFZ_V2.28.24_{stamp_start}-{stamp_end}.zip"
    ...
    info={"batch_id":batch_id,"filename":filename,"created_utc":now.isoformat(),"data_start_utc":(first or now).isoformat(),
          "data_end_utc":(last or now).isoformat(),"source_bytes":source_bytes,"zip_bytes":tmp.stat().st_size,
          "build_seconds":round(time.perf_counter()-t0,3),"batch_path":str(batch.parent)}
    (RDFZ_DIR/"RDFZ_LAST_EXPORT.json").write_text(json.dumps(info,indent=2),encoding="utf-8")
    return FileResponse(str(tmp),media_type="application/zip",filename=filename,headers={"Cache-Control":"no-store","X-RDFZ-Batch-ID":batch_id})
...
def instrumentation_status():
    ...
    return JSONResponse({"ok":True,"version":"V2.28.24 / V10.39.14","rdfz_dir":str(RDFZ_DIR),
