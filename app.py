...
def build_state():
    ...
    return clean({
        "status":"ok","version":"V2.28.33 / V10.39.19",
        "server_time":now,
        "market":{
...
app = FastAPI(title="BTC15M V2.28.19 / V10.39.9 Server Auto Paper V1 + SID-Scoped Kalshi WS", lifespan=lifespan)
...
PROSPECTIVE_LOG_PATH = Path(os.getenv("PROSPECTIVE_LOG_PATH", str(RDFZ_DIR / "BTC15M_V10_39_14_PROSPECTIVE_ARCHITECT_SNAPSHOTS.ndjson")))
...
def prospective_instrumentation_tick(d):
    ...
    if now-_LAST_PROSPECTIVE_AT >= PROSPECTIVE_MIN_INTERVAL_S:
        ...
        rec={
            ...
            "app_version":"V2.28.33","research_version":"V10.39.19",
    ...
    if now-_LAST_WS_HEALTH_AT >= WS_HEALTH_INTERVAL_S:
        ...
        _append_ndjson(WS_HEALTH_LOG_PATH,WS_HEALTH_LOG_LOCK,{
            "schema":"BTC15M_WS_HEALTH_V1","app_version":"V2.28.22",
...
def _rdfz_record_timestamp(obj):
    """Best-effort event timestamp extraction for export accounting only."""
    if not isinstance(obj,dict): return None
    keys=("timestamp","timestamp_utc","ts","ts_utc","time_utc","event_time","event_time_utc","created_utc","source_ts","source_timestamp","received_utc","server_ts")
    for k in keys:
        v=obj.get(k)
...
def _rdfz_file_stats(p):
    row={"name":p.relative_to(RDFZ_DIR).as_posix(),"bytes":p.stat().st_size,
    ...
    try:
        with p.open("r",encoding="utf-8",errors="replace") as f:
            for line in f:
                ...
                n+=1
                try: d=_rdfz_record_timestamp(json.loads(line))
...
def _rdfz_stats(files):
    rows=[]; times=[]
    for p in files:
        try:
            r=_rdfz_file_stats(p); rows.append(r)
...
def _rdfz_range(files):
    # Record timestamps are authoritative. Filesystem mtimes are fallback only.
    _,first,last=_rdfz_stats(files)
...
def _rdfz_manifest(files=None):
    ...
    rows,first,last=_rdfz_stats(files)
    return {"schema":"RDFZ_V2","app_version":"V2.28.33","research_version":"V10.39.19",
...
def rdfz_download():
    # V2.28.27 FAST EXPORT: rotate first so live logging immediately resumes,
    # then package the frozen batch with ZIP_STORED (no CPU-heavy compression).
    RDFZ_DIR.mkdir(parents=True,exist_ok=True)
    active=list(_rdfz_active_files())
    now=datetime.now(timezone.utc)
    first,last=_rdfz_range(active)
    stamp_start=(first or now).strftime("%Y%m%d_%H%M%S")
    stamp_end=(last or now).strftime("%Y%m%d_%H%M%S")
    batch_id=f"WEB_V2.28.33_{stamp_start}-{stamp_end}_{now.strftime('%f')}"
    batch_root=RDFZ_DIR/"_exports"/batch_id
    batch=batch_root/"RDFZ"
    batch.mkdir(parents=True,exist_ok=True)
    moved=[]
    for src in active:
        try:
            rel=src.relative_to(RDFZ_DIR); dst=batch/rel; dst.parent.mkdir(parents=True,exist_ok=True)
            os.replace(src,dst); moved.append(dst)
        except Exception: pass
    moved_rows,moved_first,moved_last=_rdfz_stats(moved)
    first=moved_first or first; last=moved_last or last
    manifest={"schema":"RDFZ_EXPORT_BATCH_V4_RELIABLE","app_version":"V2.28.33","research_version":"V10.39.19",
              "batch_id":batch_id,"generated_at_utc":now.isoformat(),
              "data_start_utc":(first or now).isoformat(),"data_end_utc":(last or now).isoformat(),
              "compression":"ZIP_STORED","file_count":len(moved_rows),
              "total_rows":sum((r.get("rows") or 0) for r in moved_rows),"files":moved_rows}
    (batch/"RDFZ_MANIFEST.json").write_text(json.dumps(manifest,indent=2),encoding="utf-8")
    filename=f"BTC15M_WEB_RDFZ_V2.28.33_{stamp_start}-{stamp_end}.zip"
    archive=batch_root/filename
    t0=time.perf_counter(); source_bytes=0
    with zipfile.ZipFile(archive,"w",zipfile.ZIP_STORED,allowZip64=True) as z:
        for fp in sorted(batch.rglob("*")):
            if fp.is_file():
                source_bytes+=fp.stat().st_size; z.write(fp,arcname=f"RDFZ/{fp.relative_to(batch).as_posix()}")
    # ZIP verified before removing duplicated frozen staging data.
    ...
    info={"batch_id":batch_id,"filename":filename,"archive_path":str(archive),"created_utc":now.isoformat(),
          "data_start_utc":(first or now).isoformat(),"data_end_utc":(last or now).isoformat(),
          "source_bytes":source_bytes,"zip_bytes":archive.stat().st_size,"compression":"ZIP_STORED",
          "build_seconds":round(time.perf_counter()-t0,3),"batch_path":str(batch_root)}
    (RDFZ_DIR/"RDFZ_LAST_EXPORT.json").write_text(json.dumps(info,indent=2),encoding="utf-8")
    return FileResponse(str(archive),media_type="application/zip",filename=filename,
                        headers={"Cache-Control":"no-store","X-RDFZ-Batch-ID":batch_id,"X-RDFZ-Export-Mode":"fast-stored"})

@app.get("/api/rdfz/exports")
def rdfz_exports():
    return JSONResponse({"ok":True,"exports":_rdfz_existing_exports()},headers={"Cache-Control":"no-store"})
...
def rdfz_download_last():
    # Re-download the already prepared archive without rotating or rebuilding.
    lp=RDFZ_DIR/"RDFZ_LAST_EXPORT.json"
    if not lp.exists(): return JSONResponse({"ok":False,"error":"no_completed_export"},status_code=409)
    try: info=json.loads(lp.read_text(encoding="utf-8"))
    except Exception as exc:return JSONResponse({"ok":False,"error":"invalid_export_state","detail":str(exc)},status_code=500)
    archive=Path(str(info.get("archive_path") or ""))
    if not archive.exists() or not archive.is_file():
        return JSONResponse({"ok":False,"error":"cached_export_not_found"},status_code=409)
    return FileResponse(str(archive),media_type="application/zip",filename=info.get("filename") or archive.name,
                        headers={"Cache-Control":"no-store","X-RDFZ-Batch-ID":str(info.get("batch_id") or ""),"X-RDFZ-Export-Mode":"cached"})
...
def rdfz_clear_exported():
    ...
    batch_dir=RDFZ_DIR/"_exports"/str(batch_id)
    if not batch_id or not batch_dir.exists(): return JSONResponse({"ok":False,"error":"export_batch_not_found"},status_code=409)
    freed=sum((p.stat().st_size for p in batch_dir.rglob("*") if p.is_file()),0)
    shutil.rmtree(batch_dir)
    info["cleared_utc"]=datetime.now(timezone.utc).isoformat(); info["cleared_bytes"]=freed
    lp.write_text(json.dumps(info,indent=2),encoding="utf-8")
    return JSONResponse({"ok":True,"cleared_batch_id":batch_id,"cleared_bytes":freed,"storage":_storage_status()},headers={"Cache-Control":"no-store"})
...
def instrumentation_status():
    ...
    return JSONResponse({"ok":True,"version":"V2.28.33 / V10.39.19","rdfz_dir":str(RDFZ_DIR),
