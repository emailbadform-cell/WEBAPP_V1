...
def summarize_book(book,prev=None):
    ...
    pressure="BUY" if score>=0.20 else "SELL" if score<=-0.20 else "NEUTRAL"
    ...
    while MX_RUNNING:
        ...
        for ex in ("coinbase","kraken","bitstamp","gemini"):
            try:
                ...
                errs[ex]=str(e)
        with MX_LOCK:
            MOF_LATEST={"venues":venues,"combined":combine_books(venues),"ts":time.time()}
            MOF_ERROR=errs
...
def build_state():
    ...
    return clean({
        ...
        "multi_exchange":mx,
        "multi_orderflow":{
            **(dict(MOF_LATEST) if MOF_LATEST else {}),
            "errors":dict(MOF_ERROR) if MOF_ERROR else {}
        },
    })
