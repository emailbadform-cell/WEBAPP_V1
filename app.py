...
def build_state():
    ...
    return clean({
        "status":"ok","version":"V2.22 / V10.34",
        "server_time":now,
        "market":{
...
app = FastAPI(title="BTC15M V2.22 / V10.34 Ghost + L2 + Predictive Decision", lifespan=lifespan)
...
def health():
    ...
    return JSONResponse({"ok":ok,"version":"V2.22 / V10.34","cache_age_s":age,"cache_error":err}, status_code=200 if ok else 503, headers={"Cache-Control":"no-store"})
