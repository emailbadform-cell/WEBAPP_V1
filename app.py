...
def build_state():
    ...
    return clean({
        "status":"ok","version":"V2.28.34 / V10.39.19",
        "server_time":now,
        "market":{
...
def prospective_instrumentation_tick(d):
    ...
    if now-_LAST_PROSPECTIVE_AT >= PROSPECTIVE_MIN_INTERVAL_S:
        ...
        rec={
            ...
            "app_version":"V2.28.34","research_version":"V10.39.19",
...
def _rdfz_manifest(files=None):
    ...
    return {"schema":"RDFZ_V2","app_version":"V2.28.34","research_version":"V10.39.19",
...
def rdfz_download():
    """V2.28.34 cumulative, non-destructive export.

    Active RDFZ files are NEVER rotated/moved by download. Every export therefore
    contains all data still present in RDFZ_DIR, including data exported earlier.
    The ZIP is built in the runtime temp directory and removed after the response.
    """
    RDFZ_DIR.mkdir(parents=True,exist_ok=True)
    active=list(_rdfz_active_files())
    now=datetime.now(timezone.utc)
    rows,first,last=_rdfz_stats(active)
    stamp_start=(first or now).strftime("%Y%m%d_%H%M%S")
    stamp_end=(last or now).strftime("%Y%m%d_%H%M%S")
    filename=f"BTC15M_WEB_RDFZ_V2.28.34_{stamp_start}-{stamp_end}.zip"
    fd,tmp_name=tempfile.mkstemp(prefix="btc15m_rdfz_",suffix=".zip")
    os.close(fd)
    archive=Path(tmp_name)
    manifest={"schema":"RDFZ_EXPORT_V5_CUMULATIVE_NONDESTRUCTIVE","app_version":"V2.28.34","research_version":"V10.39.19",
              "generated_at_utc":now.isoformat(),
              "data_start_utc":(first or now).isoformat(),"data_end_utc":(last or now).isoformat(),
              "compression":"ZIP_STORED","export_mode":"cumulative_nondestructive",
              "source_files_preserved":True,"file_count":len(rows),
              "total_rows":sum((r.get("rows") or 0) for r in rows),"files":rows}
    ...
    try:
        with zipfile.ZipFile(archive,"w",zipfile.ZIP_STORED,allowZip64=True) as z:
            ...
            for fp in active:
                ...
                try:
                    ...
                    z.write(fp,arcname=f"RDFZ/{fp.relative_to(RDFZ_DIR).as_posix()}")
        ...
        raise
    info={"filename":filename,"created_utc":now.isoformat(),
          "data_start_utc":(first or now).isoformat(),"data_end_utc":(last or now).isoformat(),
          "source_bytes":source_bytes,"zip_bytes":archive.stat().st_size,"compression":"ZIP_STORED",
          "build_seconds":round(time.perf_counter()-t0,3),"export_mode":"cumulative_nondestructive",
    ...
    return FileResponse(str(archive),media_type="application/zip",filename=filename,
                        headers={"Cache-Control":"no-store","X-RDFZ-Export-Mode":"cumulative-nondestructive"},
                        background=BackgroundTask(lambda: archive.unlink(missing_ok=True)))

@app.get("/api/rdfz/exports")
def rdfz_exports():
    # V2.28.34 no longer stores server-side export copies.
...
def rdfz_download_last():
    # V2.28.34 exports are intentionally not retained server-side. Generate a
...
def rdfz_clear_exported():
    return JSONResponse({"ok":True,"cleared_bytes":0,"mode":"cumulative_nondestructive","note":"No duplicate server-side export cache is created in V2.28.34."},headers={"Cache-Control":"no-store"})
...
def instrumentation_status():
    ...
    return JSONResponse({"ok":True,"version":"V2.28.34 / V10.39.19","rdfz_dir":str(RDFZ_DIR),
