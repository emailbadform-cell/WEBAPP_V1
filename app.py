...
def build_state():
    ...
    return clean({
        "status":"ok","version":"V2.28.22 / V10.39.13",
        "server_time":now,
        "market":{
...
# V2.28.22 / V10.39.13: instrumentation-only prospective recorder.
# No prediction or Auto Paper policy authority is changed by this layer.
PROSPECTIVE_LOG_PATH = Path(os.getenv("PROSPECTIVE_LOG_PATH", str(RDFZ_DIR / "BTC15M_V10_39_13_PROSPECTIVE_ARCHITECT_SNAPSHOTS.ndjson")))
...
def prospective_instrumentation_tick(d):
    ...
    if now-_LAST_PROSPECTIVE_AT >= PROSPECTIVE_MIN_INTERVAL_S:
        ...
        rec={
            ...
            "app_version":"V2.28.22","research_version":"V10.39.13",
...
def _rdfz_manifest():
    ...
    for p in sorted(RDFZ_DIR.rglob("*")):
        if p.is_file() and p.name not in ("RDFZ_DOWNLOAD.zip","RDFZ_MANIFEST.json"):
            try:
                files.append({"name":p.relative_to(RDFZ_DIR).as_posix(),"bytes":p.stat().st_size,
                              "modified_utc":datetime.fromtimestamp(p.stat().st_mtime,timezone.utc).isoformat()})
            except Exception: pass
    return {"schema":"RDFZ_V1","app_version":"V2.28.22","research_version":"V10.39.13",
...
def rdfz_status():
    ...
    return JSONResponse({"ok":True,**manifest},headers={"Cache-Control":"no-store"})

@app.get("/api/rdfz/download")
def rdfz_download():
    RDFZ_DIR.mkdir(parents=True,exist_ok=True)
    manifest=_rdfz_manifest()
    (RDFZ_DIR/"RDFZ_MANIFEST.json").write_text(json.dumps(manifest,indent=2),encoding="utf-8")
    tmp=Path(tempfile.gettempdir())/"RDFZ_DOWNLOAD.zip"
    if tmp.exists(): tmp.unlink()
    root_name="RDFZ"
    with zipfile.ZipFile(tmp,"w",zipfile.ZIP_DEFLATED,allowZip64=True) as z:
        for p in sorted(RDFZ_DIR.rglob("*")):
            if p.is_file() and p.name!="RDFZ_DOWNLOAD.zip":
                z.write(p,arcname=f"{root_name}/{p.relative_to(RDFZ_DIR).as_posix()}")
    return FileResponse(str(tmp),media_type="application/zip",filename="RDFZ.zip",
                        headers={"Cache-Control":"no-store"})
...
def instrumentation_status():
    ...
    return JSONResponse({"ok":True,"version":"V2.28.22 / V10.39.13","rdfz_dir":str(RDFZ_DIR),
