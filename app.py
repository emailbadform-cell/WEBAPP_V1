import os, math, json, threading, time, tempfile, uuid
...
def build_state():
    ...
    return clean({
        "status":"ok","version":"V2.28.19 / V10.39.9",
        "server_time":now,
        "market":{
...
async def lifespan(app: FastAPI):
    ...
    KALSHI_MARKET_STREAM = KalshiMarketStream(core.KALSHI_WS_URL, _kalshi_ws_auth_headers,
        event_log_path=str(Path(__file__).with_name("BTC15M_KALSHI_WS_EVENTS.ndjson")), on_event=_kalshi_ws_event)
...
TRADE_EVENT_LOG_PATH = Path(os.getenv("TRADE_EVENT_LOG_PATH", str(Path(__file__).with_name("BTC15M_AUTO_PAPER_EVENTS.ndjson"))))
TRADE_EVENT_LOCK = threading.Lock()
CLIENT_TELEMETRY_LOG_PATH = Path(os.getenv("CLIENT_TELEMETRY_LOG_PATH", "/tmp/btc15m_client_latency.ndjson"))
CLIENT_TELEMETRY_LOCK = threading.Lock()


# V2.28.19: server-owned frozen Auto Paper V1. Manual browser paper trading is intentionally separate.
...
AUTO_PAPER_STATE_PATH=Path(os.getenv("AUTO_PAPER_STATE_PATH",str(Path(__file__).with_name("BTC15M_AUTO_PAPER_STATE.json"))))
...
def _trade_context(d):
    ...
    return {"ticker":m.get("ticker"),"brti":m.get("brti"),"target":m.get("target"),"seconds_left":m.get("seconds_left"),"yes_bid":em.get("yes_bid"),"yes_ask":em.get("yes_ask"),"no_bid":em.get("no_bid"),"no_ask":em.get("no_ask"),"quote_age_ms":em.get("quote_age_ms"),"book_healthy":em.get("book_healthy"),"book_sid":em.get("book_sid"),"book_seq":em.get("book_seq"),"decision":ds.get("decision"),"decision_confidence":ds.get("confidence"),"settlement_prediction":st.get("prediction"),"settlement_confidence":st.get("confidence"),"flip_count":fl.get("flip_count"),"reversal":xr.get("reversal"),"profit_risk_level":pr.get("level"),"profit_risk_score":pr.get("score")}

def _auto_event(kind,d,extra=None,pos=None):
    pos=pos or AUTO_PAPER.get("position") or {}
    payload={"event_type":kind,"source":"SERVER_AUTO_V1","policy":AUTO_PAPER_POLICY["version"],"trade_id":pos.get("trade_id"),"side":pos.get("side"),"entry":pos.get("entry"),"stake":pos.get("stake",100.0),"context":_trade_context(d)}
    ...
    with AUTO_PAPER_LOCK:
        ...
        if ticker!=pos.get("ticker") or float(m.get("seconds_left") or 0)<=0:
            ...
            if brti is not None and target is not None:
                ...
                _auto_event("EXIT",d,{"exit":xp,"pnl":pnl,"reason":"EXPIRY / ROLLOVER","peak_bid":pos["peak_bid"],"trough_bid":pos["trough_bid"]},pos); AUTO_PAPER["last_exit"]={"trade_id":pos["trade_id"],"pnl":pnl,"reason":"EXPIRY / ROLLOVER","at":now}; AUTO_PAPER["position"]=None; _save_auto_state()
            return
