...
def prospective_instrumentation_tick(d):
    ...
    if now-_LAST_PROSPECTIVE_AT >= PROSPECTIVE_MIN_INTERVAL_S:
        ...
        if os.getenv("BTC15M_SAFE_TELEMETRY", "0") == "1":
            try:
                from telemetry_contract import safe_snapshot, append_jsonl
                safe=safe_snapshot({**d,"rollover_telemetry":rec["rollover_telemetry"]},platform="web")
                append_jsonl(str(PROSPECTIVE_LOG_PATH)+".safe.jsonl",safe)
            except Exception:
                pass  # passive logging must never interrupt the dashboard
