...
class KalshiMarketStream:
    def __init__(self, ws_url, auth_headers_fn, event_log_path=None, on_event=None):
        ...
        self.reconnects=0; self.sequence_gaps=0; self.generation=0; self.sid_seq={}
        self.reconnect_reasons={'subscription_change':0,'network_or_ws_error':0}
    ...
    def set_markets(self, tickers):
        ...
        clean=clean[:3]
        with self.lock:
            # Treat subscription membership as a set. Caller ordering can vary between
            # state builds and must not force a reconnect. Also retire completed-market
            # state so books/tickers/trades/sid maps cannot grow for days.
            if set(clean)!=set(self.desired):
                self.desired=clean; self.generation+=1
            keep=set(clean)
            self.books={k:v for k,v in self.books.items() if k in keep}
            self.tickers={k:v for k,v in self.tickers.items() if k in keep}
            self.trades={k:v for k,v in self.trades.items() if k in keep}
            live_sids={v.get('sid') for v in self.books.values() if v.get('sid') is not None}
            self.sid_seq={k:v for k,v in self.sid_seq.items() if k in live_sids}
    def _emit(self, kind, msg, seq=None, sid=None):
        now=datetime.now(timezone.utc).isoformat()
        rec={'kind':kind,'server_received_utc':now,'sid':sid,'seq':seq,'msg':msg}
    ...
    def process_message(self, data):
        typ=data.get('type'); msg=data.get('msg') or {}; seq=data.get('seq'); sid=data.get('sid'); ticker=msg.get('market_ticker')
        if typ=='orderbook_snapshot' and ticker:
            ...
            with self.lock:
                self.books[ticker]={'yes':y,'no':n,'seq':seq,'sid':sid,'updated_mono':time.monotonic(),'updated_utc':datetime.now(timezone.utc).isoformat(),'healthy':True}
                if sid is not None and seq is not None:self.sid_seq[sid]=int(seq)
            self._emit(typ,msg,seq,sid); return
        if typ=='orderbook_delta' and ticker:
            with self.lock:
                ...
                # Kalshi sequence numbers belong to the WebSocket subscription (sid), not a ticker-local stream.
                # Current+next markets can therefore interleave while remaining perfectly contiguous at sid scope.
                prev=self.sid_seq.get(sid) if sid is not None else None
                if sid is not None and seq is not None and prev is not None and int(seq)!=int(prev)+1:
                    for _t,_b in self.books.items():
                        if _b.get('sid')==sid:_b['healthy']=False
                    self.sequence_gaps+=1; raise RuntimeError(f'orderbook subscription sequence gap sid={sid} {prev}->{seq}')
                if sid is not None and seq is not None:self.sid_seq[sid]=int(seq)
                ...
                b['seq']=seq; b['sid']=sid; b['updated_mono']=time.monotonic(); b['updated_utc']=datetime.now(timezone.utc).isoformat(); b['healthy']=True
            self._emit(typ,msg,seq,sid); return
    ...
    def book(self,ticker):
        ...
        return {'yes_bid':yb,'yes_ask':ya,'no_bid':nb,'no_ask':na,
                ...
                'book_seq':b.get('seq'),'book_sid':b.get('sid'),'book_healthy':bool(b.get('healthy')),'book_levels_yes':len(y),'book_levels_no':len(n)}
    def health(self):
        with self.lock:
            return {'status':self.status,'error':self.error,'markets':list(self.desired),'reconnects':self.reconnects,
                    'sequence_gaps':self.sequence_gaps,'reconnect_reasons':dict(self.reconnect_reasons),'last_event_age_s':(time.monotonic()-self.last_event_mono) if self.last_event_mono else None,
                    'sid_seq':dict(self.sid_seq),'books':{k:{'healthy':v.get('healthy'),'seq':v.get('seq'),'sid':v.get('sid'),'age_s':time.monotonic()-v.get('updated_mono',0)} for k,v in self.books.items()}}
    def _worker(self):
        backoff=.5
        ...
        while self.running:
            ...
            try:
                ...
                reason='subscription_change' if str(e)=='market subscription changed' else 'network_or_ws_error'
                with self.lock:
                    self.reconnect_reasons[reason]=self.reconnect_reasons.get(reason,0)+1
                with self.lock:
                    for t in markets:
