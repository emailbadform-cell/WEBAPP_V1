...
class SignalSuite:
    def __init__(self, base_dir):
        ...
        self._gt_minute_bucket=None; self._gt_direction='SIDEWAYS'; self._gt_candidate=None; self._gt_candidate_count=0
    @staticmethod
    ...
    def forward_state(self,ticks,current,now):
        ...
        # V2.24.4 UI clarity: keep REVERSAL as internal/research phase, but never render that word in the live Forward label.
        label='SIDEWAYS' if self._direction=='SIDEWAYS' else (f'{self._direction} — CONTINUATION' if phase=='CONTINUATION' else self._direction)
    ...
    def decision_state(self,ticks,current,target,seconds,now):
        ...
        return {'decision':side,'state':side,'p_above':p,'p_below':1-p,'confidence':abs(p-.5)*2,'architecture':'PREDICTIVE PRICE GEOMETRY — NO CURRENT-SIDE INPUT'}
    @staticmethod
    ...
    def ghost_state(self,ticks,current,now,fw):
        ...
        if e<5: proposed=fw['state']
        elif e<10: proposed=pside if pside!='FLAT' and abs(out['1']['distance_bps'] or 0)>=.10 else fw['state']
        else: proposed=pside if pside!='FLAT' else fw['state']
        stage='FORWARD' if e<5 else 'BLEND' if e<10 else 'PATH' if e<30 else 'PERSISTENCE'
        # Stateful GT direction hysteresis.  Reset at each new UTC minute, then require
        # two consecutive contrary observations before repainting the projected candle.
        # This is display/lifecycle state only; it does not feed Forward, Decision, TR,
        # MP, OF, L2, or any frozen model features.
        minute_bucket=pd.Timestamp(now).floor('min')
        if self._gt_minute_bucket!=minute_bucket:
            self._gt_minute_bucket=minute_bucket
            self._gt_direction=proposed if proposed in ('UP','DOWN') else 'SIDEWAYS'
            self._gt_candidate=None; self._gt_candidate_count=0
        elif proposed==self._gt_direction:
            self._gt_candidate=None; self._gt_candidate_count=0
        elif proposed in ('UP','DOWN','SIDEWAYS'):
            self._gt_candidate_count=self._gt_candidate_count+1 if self._gt_candidate==proposed else 1
            self._gt_candidate=proposed
            if self._gt_candidate_count>=2:
                self._gt_direction=proposed; self._gt_candidate=None; self._gt_candidate_count=0
        gt_dir=self._gt_direction
        ...
        rv=max(.05,float(f.get('rv30',0.))); return {'fixed_open_1m':out['1']['open'],'elapsed_1m_s':e,'stage':stage,'direction':gt_dir,'proposed_direction':proposed,'anti_flicker_active':self._gt_candidate is not None,'active_horizon_s':fw['horizon_s'],'uncertainty_bps':rv*math.sqrt(max(1,fw['horizon_s'])),'timeframes':out,'snapback_5m_warning':snap5,'strong_2m_snapback_warning':snap2,'candle2':c2,'candle3_active':False}
