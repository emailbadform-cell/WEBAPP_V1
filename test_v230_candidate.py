...
def state(sec, decision='ABOVE', settlement='BELOW', ask=.60, bid=.58, brti=101, target=100, ticker='T1'):
    return {'status':'ok','market':{'ticker':ticker,'seconds_left':sec,'brti':brti,'target':target,'brti_age_s':0.1},
            'decision':{'decision':decision,'confidence':70},'settlement':{'prediction':settlement,'confidence':60},
            'execution_market':{'yes_ask':ask,'yes_bid':bid,'no_ask':1-bid,'no_bid':1-ask,'book_healthy':True,'quote_age_ms':100.0},
