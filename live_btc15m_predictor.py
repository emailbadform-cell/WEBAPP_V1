#!/usr/bin/env python3
"""
BTC15M AUTO LIVE V10.18 — MAJOR RESEARCH / DECISION V2 / OF MICRO
===========================================
...
RESEARCH_LOG_FILE = HERE / "BTC15M_V10_19_TR_RESEARCH_LOG.csv"
...
def build_dashboard(r, now=None, orderflow=None, err=None, of_err=None, mof=None, mof_err=None):
    ...
    add=L.append
    add("="*76)
    add("KALSHI KXBTC15M — V10.19 TR MAJOR RESEARCH")
    add("="*76)
    ...
    add(f"Early warning:    {_c(es,esc)}   |   Score {float(early.get('score',0.0)):+.3f}")
    ...
    add(f"OF micro (~10s):  {_c(micro['signal'],mc)}   |   {micro['strength']}   |   score {micro['score']:+.3f}")
    ...
    venues=(mof or {}).get("venues") or {}
    for ex,label in (("coinbase","Coinbase"),("kraken","Kraken"),("bitstamp","Bitstamp"),("gemini","Gemini")):
        v=venues.get(ex) or {}
    ...
    tr=target_reachability(r['distance'],r['atr1m_14'],sec,r.get('prediction'))
    trof=target_reachability_of_context(r,mof,early)
    add("TARGET REACHABILITY (TR) — V10.19 CALIBRATED")
    ...
    add(f"Raw model cross:  {100*tr['raw_cross_estimate']:.1f}%   |   OF target context: {trof['state']}")
    add(f"OF toward score:  {trof['toward_score']:+.3f}   |   OF does not alter calibrated TR probability")
    if tr.get('role'):
