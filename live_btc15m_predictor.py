...
def settlement_window_state(ticks, current, target, close_time, now, preserved_seconds=None):
    try:
        ...
        q=(pd.DataFrame([{'time':k,'value':v} for k,v in (preserved_seconds or {}).items()]) if preserved_seconds else (ticks[['time','value']].copy() if ticks is not None and len(ticks) else pd.DataFrame(columns=['time','value'])))
