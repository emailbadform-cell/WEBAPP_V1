...
RESEARCH_LOG_FILE = HERE / "BTC15M_V10_37_RESEARCH_LOG.csv"
...
def predictive_decision_signal(r, seconds_left, ticks, now=None):
    """V10.35 no-current-side predictive Decision used by research logging.
...
def decision_signal(r, seconds_left):
    """Compatibility wrapper for stale research/dashboard callers.

    V10.35 removed the legacy current-side Decision function.  Reuse the
    predictive Decision already attached to the live row when available; if a
    caller reaches this path before the cache is attached, compute the same
    frozen no-current-side Decision with neutral return-history features rather
    than falling back to Current Side.
    """
    cached=(r or {}).get('_predictive_decision') if isinstance(r,dict) else None
    if isinstance(cached,dict) and cached.get('decision') in ('ABOVE','BELOW'):
        return cached
    return predictive_decision_signal(r, seconds_left, None, datetime.now(timezone.utc))
...
def signal_fusion_research(r, mof, early):
    ...
    """
    ds=predictive_decision_signal(r,max(0.0,(r["close_time"]-datetime.now(timezone.utc)).total_seconds()),None,datetime.now(timezone.utc))
    dec=ds["decision"]
...
def settlement_confirmation_research(r, seconds_left=None):
    ...
    ds=predictive_decision_signal(r,float(seconds_left or 0.0),None,datetime.now(timezone.utc))
...
def build_dashboard(r, now=None, orderflow=None, err=None, of_err=None, mof=None, mof_err=None):
    ...
    add("Chart remains pure; Decision is predictive/no-current-side; OF/L2 remain research/context only.")
    add("")
...
class LiveEngine(LiveEngine):
    ...
    def __init__(self,*a,**kw):
        ...
        self.next_market_found_at=None; self.rollover_triggered_at=None
        ...
        self.history_thread=None; self.history_ready=False
    ...
    def _ensure_market(self):
        ...
        # V2.16: begin direct next-contract prefetch 90 seconds before expiry.
        # Retry the deterministic ticker every second; broad discovery is only fallback.
        if remaining<=90 and self.prefetch_started_at is None:
            self.prefetch_started_at=now.isoformat()

        if remaining<=90 and self.next_market is None and time.monotonic()-self.last_direct_prefetch>=1.0:
        ...
        if remaining<=90 and self.next_market is None and time.monotonic()-self.last_market_discovery>=2.0:
            ...
            if time.monotonic()-self.last_direct_prefetch>=0.35:
                ...
                try:
                    ...
                    if ticker:
                        ...
                        m=payload.get('market',payload)
                        close=parse_dt(m.get('close_time') or m.get('expiration_time')); op=parse_dt(m.get('open_time'))
                        target=float(m.get('floor_strike',m.get('functional_strike')))
    ...
    def _history_backfill_worker(self):
        # V2.24.7: historical BRTI warmup is independent from the live socket and
        # market discovery. A slow REST history call must never block first live data.
        while self.running and not self.history_ready:
            try:
                hist=fetch_brti_history(hours=6)
                if hist is not None and len(hist):
                    # Merge any live ticks already accumulated while history was loading.
                    with self.lock:
                        live=self.ticks.copy() if self.ticks is not None else None
                ...
                self._set(error='BRTI history warmup retrying: '+str(e))
            time.sleep(1.0)

    def worker(self):
        # V2.24.7 fast-init architecture:
        # 1) start authenticated 5Hz BRTI immediately,
        # 2) backfill 6h BRTI history in parallel,
        # 3) retry Kalshi market discovery forever instead of killing the worker.
        self.ws_thread=threading.Thread(target=self.brti_websocket_worker,daemon=True)
        self.ws_thread.start()
        self.history_thread=threading.Thread(target=self._history_backfill_worker,daemon=True)
        self.history_thread.start()

        while self.running:
            try:
                # Active-contract discovery is retryable. Startup is never terminal.
                if self.market is None:
                    try:
                        self._ensure_market()
                    except Exception as e:
                        self._set(error='Kalshi market discovery retrying: '+str(e))
                        time.sleep(0.25)
                        continue
                else:
                    self._ensure_market()
                ...
                # Process the newest live tick as soon as we have enough history for
                # feature construction. If history is still warming, retain the live
                # tick so the UI can transition immediately when backfill completes.
                if latest is not None:
                    key=str(latest['time'])
                    if key!=self.last_processed_ws_ts:
                        try:
                            r=self._append_live_tick(latest['value'],latest['time'])
                            self.last_processed_ws_ts=key
                            self._set(result=r,error=None)
                        except Exception as e:
                            # Most commonly insufficient warmup history. Do not mark the
                            # tick processed; retry it after backfill advances.
                            self._set(error='Live BRTI warming model context: '+str(e))
                ...
                if stale>2.5 and time.monotonic()-self.last_rest_fallback>=1.0:
                    ...
                    try:
                        # REST fallback also doubles as a bootstrap source if the WS is
                        # temporarily unavailable. It never terminates the main worker.
                        if self.ticks is None:
                            recent,_=fetch_brti_recent_values()
                            self.ticks=recent
                            self.candles=brti_ticks_to_1m(self.ticks)
                        ...
                        self._set(error='BRTI bootstrap/recovery retrying: '+str(e))
            except Exception as e:
                self._set(error=str(e))
            time.sleep(0.20)
