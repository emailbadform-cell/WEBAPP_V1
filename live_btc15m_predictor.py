...
def fetch_active_kalshi_market(now=None):
    ...
    # V2.28.45: distinguish "ticker exists but strike/target is not published yet"
    # from "no current contract".  Prediction still waits for the target because
    # target-relative architectures require it, but discovery no longer turns this
    # normal Kalshi handoff state into a terminal initialization failure.
    if expected_ticker:
        try:
            payload=_public_get(f"/markets/{expected_ticker}",{})
            m=payload.get("market",payload)
            if isinstance(m,dict) and str(m.get("ticker") or expected_ticker)==expected_ticker:
                close=parse_dt(m.get("close_time") or m.get("expiration_time"))
                if close is not None and close>now and _market_target(m) is None:
                    target_pending=True
    ...
    if target_pending:
        raise RuntimeError(
            "TICKER_FOUND_TARGET_PENDING — current KXBTC15M contract exists but its strike/target is not published yet. "
            f"Expected {expected_ticker or 'unknown'}; automatic discovery retry active"
        )
    raise RuntimeError(
        "WAITING_FOR_CURRENT_KXBTC15M — no usable current contract resolved yet; automatic discovery retry active. "
...
def compute_features(df,target,minutes_left,current_price=None):
    ...
    m1=trend_state(d); prev_m1_trend=trend_state(d.iloc[:-1]); m5=trend_state(f5); m15=trend_state(f15)
    ...
    f={
      ...
      "m1_trend":m1,"prev_m1_trend":prev_m1_trend,"m5_trend":m5,"m15_trend":m15,
...
def predict_from_data(market,candles,current,source="BRTI", quote_time=None):
    ...
    return {
      ...
      "m1_trend":state["m1"],"prev_m1_trend":f.get("prev_m1_trend",0),"m5_trend":state["m5"],"m15_trend":state["m15"],
      ...
      "macd_line":f["macd_line"],"macd_signal":f["macd_signal"],"macd_hist":f["macd_hist"],"macd_hist_velocity":f.get("macd_hist_velocity",0.0),
      "body":f["body"],"wick_up":f["wick_up"],"wick_dn":f["wick_dn"],
      "range5_abs":f["range5"]*btc,"range15_abs":f["range15"]*btc,
    }


# V10.39.17 / V2.28.29 settlement-window research.
# This is an internal reconstruction from observed BRTI, not an official Kalshi settlement value.
def settlement_window_state(ticks, current, target, close_time, now, preserved_seconds=None):
    try:
        ...
        # Merge the persistent source-second accumulator with the engine tick buffer.
        # V10.39.24/V2.28.40 accidentally *replaced* ticks whenever even one
        # preserved second existed, which capped Final Target at ~0-2 samples.
        frames=[]
        if ticks is not None and len(ticks):
            frames.append(ticks[['time','value']].copy())
        if preserved_seconds:
            frames.append(pd.DataFrame([{'time':k,'value':v} for k,v in preserved_seconds.items()]))
        q=(pd.concat(frames,ignore_index=True) if frames else pd.DataFrame(columns=['time','value']))
        q['time']=pd.to_datetime(q['time'],utc=True,errors='coerce'); q['value']=pd.to_numeric(q['value'],errors='coerce')
        q=q.dropna().sort_values('time'); effective_end=min(nowp,closep-pd.Timedelta(microseconds=1)); q=q[(q.time>=start)&(q.time<=effective_end)]
        if preserved_seconds is None and sec<=60 and current is not None and nowp<closep:
        ...
        # V10.39.17: parallel reconstruction diagnostics. Production/proxy value remains last-tick/source-time.
        first_avg=mean_avg=None
        missing_seconds=[]
        if len(q):
            qq=q.copy(); qq['second']=qq.time.dt.floor('s')
            first_avg=float(qq.groupby('second',as_index=False).head(1).value.astype(float).mean())
            mean_avg=float(qq.groupby('second').value.mean().astype(float).mean())
            expected_end=min(nowp.floor('s'),closep.floor('s')-pd.Timedelta(seconds=1)); expected=pd.date_range(start=start.floor('s'),end=expected_end,freq='1s',tz='UTC') if expected_end>=start.floor('s') else pd.DatetimeIndex([])
            present=set(qq['second'].tolist()); missing_seconds=[x.isoformat() for x in expected if x not in present]
        return {'schema':'FINAL_TARGET_PROXY_V2_DIAGNOSTIC','proxy':True,'label':'Final Target (Est.)','window_start_utc':start.isoformat(),'window_end_utc':closep.isoformat(),
                'seconds_remaining':sec,'sample_count':n,'remaining_samples_est':remaining,'missing_second_count':len(missing_seconds),'missing_seconds_utc':missing_seconds[:120],
                'sampling_rule':'SOURCE_TIMESTAMP_LAST_TICK_PER_SECOND','final_target':avg,'first_tick_avg':first_avg,'per_second_mean_avg':mean_avg,'distance_to_target':dist,
...
def gt_final_target_challengers(r):
    """V10.39.17 research-only frozen FT-context sweep; never changes GT control or trading authority."""
    try:
        path=r.get('_gt_common_anchor_path') or {}; ft=r.get('_final_target') or {}
        eps=path.get('G_endpoints') or []; control=float(eps[14]) if len(eps)>=15 else None
        f=float(ft.get('final_target')) if ft.get('final_target') is not None else None
        if control is None or f is None:
            return {'schema':'GT15_FT_CHALLENGERS_V1','research_only':True,'authority':False,'control':control,'final_target_est':f,'candidates':{}}
        gap=f-control
        return {'schema':'GT15_FT_CHALLENGERS_V1','research_only':True,'authority':False,'target':'ACTUAL_FUTURE_BRTI_GT15','control':control,'final_target_est':f,
                'ft_minus_control':gap,'sample_count':ft.get('sample_count'),'required_remaining_brti':ft.get('required_remaining_brti'),
                'candidates':{f'FT_BLEND_{int(a*100):02d}':control+a*gap for a in (0.10,0.25,0.50)}}
    except Exception as e:
        return {'schema':'GT15_FT_CHALLENGERS_V1','research_only':True,'authority':False,'error':str(e)}
...
class LiveEngine:
    ...
    def kalshi_final_target_snapshot(self):
        with self.lock:
            if not self.kalshi_final_target: return None
            out=dict(self.kalshi_final_target)
            out['age_ms']=max(0.0,(time.monotonic()-self.kalshi_final_target_last_mono)*1000.0) if self.kalshi_final_target_last_mono else None
            return out
...
def trend_transition_challenger_v2(r, ticks=None, now=None):
    """Frozen prospective Trend Transition V2; research/shadow only.

    ARMED before second 20 when the developing 1m HH/HL or LH/LL structure
    opposes the previous completed 1m structural trend. TRIGGERED at/after
    second 20. MACD and 5m/15m are context only, never gates.
    """
    now = now or datetime.now(timezone.utc)
    sec = int(getattr(now, 'second', 0))
    prev_tr = int(r.get('prev_m1_trend') or 0)
    dev_tr = int(r.get('m1_trend') or 0)
    opposite = bool(prev_tr != 0 and dev_tr == -prev_tr)
    desired = 'BULLISH' if prev_tr < 0 else 'BEARISH' if prev_tr > 0 else 'NEUTRAL'
    mh=float(r.get('macd_hist') or 0.0); mv=float(r.get('macd_hist_velocity') or 0.0)
    macd_context = ('CONFIRMING' if desired=='BULLISH' and mv>0 else
                    'CONFIRMING' if desired=='BEARISH' and mv<0 else
                    'OPPOSING' if desired!='NEUTRAL' and mv!=0 else 'NEUTRAL')
    if not opposite:
        state='CURRENT TREND HOLD' if prev_tr else 'NEUTRAL / WAIT'
        direction='NEUTRAL'; triggered=False; armed=False; conf=0.0
    elif sec < 20:
        state=desired+' ARMED'; direction=desired; triggered=False; armed=True; conf=0.9254
    else:
        state=desired+' TRIGGERED'; direction=desired; triggered=True; armed=True; conf=0.9335
    return {'version':'TREND_TRANSITION_CHALLENGER_V2','state':state,'direction':direction,
            'confidence':round(conf,4),'score':round(conf,4),'armed':armed,'triggered':triggered,
            'second_in_minute':sec,'horizon_s':40,'previous_m1_trend':prev_tr,'developing_m1_trend':dev_tr,
            'macd_context':macd_context,'macd_hist':mh,'macd_hist_velocity':mv,
            'm5_context':r.get('m5_trend'),'m15_context':r.get('m15_trend'),
            'authority':False,'research_only':True,
            'historical_reference':'V2 frozen: second>=20 opposite developing 1m structure; 309/331 strict reconstruction (93.35%); newest block 90/95 (94.74%); prospective confirmation required'}
...
def of_assisted_decision_research(r, mof, early):
    ...
    # Secondary OF overlay uses the cached predictive Decision, never a missing
    # legacy decision field or the current-price side. No trading authority.
    predictive=(r.get('_predictive_decision') or {}) if isinstance(r,dict) else {}
    base=str(predictive.get('decision') or '').upper() if isinstance(predictive,dict) else ''
    if base not in ('ABOVE','BELOW'):
        combined=(mof or {}).get('combined') or {}
        raw_pressure=_safe_float(combined.get('pressure_score')) or 0.0
        raw_early=_safe_float((early or {}).get('score')) or 0.0
        raw_score=float(np.clip(0.65*raw_pressure+0.35*raw_early,-1.0,1.0))
        return {'base':'—','assisted':'—','state':'NO PREDICTIVE BASE',
                'risk':'UNAVAILABLE','score':raw_score,
                'of_direction':'BUY' if raw_score>=0.08 else ('SELL' if raw_score<=-0.08 else 'NEUTRAL'),
                'reason':'cached_predictive_decision_missing'}
    c=(mof or {}).get("combined") or {}
...
def _gt_common_anchor_path_capture(r, now):
    """GT-C primary: actual-replacement path; GT-B: two-step anticipation challenger.

    GT-C removes the old dependency-weighted predicted-anchor recursion.  Each GT
    body is the horizon's own frozen Outlook delta.  Consumers MUST replace the
    projected anchor with actual BRTI whenever that GT slot realizes.  Until a
    slot realizes, future slots remain causally chained from the latest known
    actual anchor.  GT-B uses the same actual anchor but looks one additional
    GT body ahead (GT15 extrapolates one extra body from GT15).
    Neither path has Decision/Settlement/trading authority.
    """
    try:
        ...
        # Adjacent one-step bodies are the GT-C forecast deltas.  No upstream
        # predicted endpoint is blended into a later GT delta.
        c_bodies=[float(raw_cum[k]-raw_cum[k-1]) for k in range(1,16)]
        c_endpoints=[]; px=g0; c_steps=[]
        for k,body in enumerate(c_bodies,1):
            op=px; px=op+body; c_endpoints.append(float(px))
            c_steps.append({'GT':k,'anchor_mode':'ACTUAL_REPLACEMENT_WHEN_AVAILABLE',
                            'open':float(op),'close':float(px),'body':float(body),
                            'rule':_GT_RULEBOOK_V1039.get(k,{})})

        # GT-B anticipation: from the same actual-replacement anchor, forecast
        # through the next GT delta as well.  GT15 has no native GT16 body, so
        # its second step is a transparent last-body persistence extrapolation.
        b_bodies=[]; b_endpoints=[]; b_steps=[]; px=g0
        for i,body in enumerate(c_bodies):
            nxt=c_bodies[i+1] if i+1<len(c_bodies) else c_bodies[-1]
            anticip=float(body+nxt)
            b_bodies.append(anticip); b_endpoints.append(float(px+anticip))
            b_steps.append({'GT':i+1,'anchor_mode':'ACTUAL_ANCHOR_FORWARD_SKIP',
                            'one_step_body':float(body),'next_body':float(nxt),
                            'anticipation_body':anticip,'close':float(px+anticip),
                            'gt16_extrapolated':bool(i==14)})
            px += body

        ticker=str(r.get('ticker') or '')
        old=_GT_PATH_LAST_V10392.get(ticker); changed=[]
        if old:
            for k,(a,b) in enumerate(zip(old.get('endpoints',[]),c_endpoints),1):
                if abs(float(a)-float(b))>=0.01: changed.append(k)
        first_changed=min(changed) if changed else None
        out={'schema':'GT_ACTUAL_REPLACEMENT_C_V1','research_only':True,'authority':False,
             'method':'GT_C_ACTUAL_REPLACEMENT','t0':nowp.isoformat(),'G0':g0,
             'G_endpoints':c_endpoints,'F_bodies':c_bodies,
             'directions':['UP' if x>0 else ('DOWN' if x<0 else 'SIDEWAYS') for x in c_bodies],
             'actual_replacement_required':True,'recursive_steps':c_steps,
             'anticipation_challenger':{'schema':'GT_ACTUAL_ANCHOR_FORWARD_B_V1','authority':False,
                 'G_endpoints':b_endpoints,'F_bodies':b_bodies,'steps':b_steps},
             'changed_GT':changed,'first_changed_GT':first_changed,'redraw':bool(changed),
             'redraw_time':nowp.isoformat() if changed else None}
        _GT_PATH_LAST_V10392[ticker]={'endpoints':c_endpoints,'time':nowp.isoformat()}
        return out
    except Exception as e:
        return {'schema':'GT_ACTUAL_REPLACEMENT_C_V1','research_only':True,'authority':False,'error':str(e)}
...
class LiveEngine(LiveEngine):
    ...
    def __init__(self,*a,**kw):
        ...
        # Kalshi's official CF Benchmarks quarter-hour final-minute accumulator.
        # This is display/research settlement context only; it does not alter Decision/GT.
        self.kalshi_final_target=None; self.kalshi_final_target_last_mono=0.0
    ...
    def _ensure_market(self):
        ...
        metadata_window = (remaining<=90 or bool(self.ws_next_ticker_seen_at))
        if metadata_window and self.next_market is None and time.monotonic()-self.last_direct_prefetch>=prefetch_retry_s:
        ...
        if remaining<=30 and self.next_market is None and time.monotonic()-self.last_market_discovery>=2.0:
    ...
    def _append_live_tick(self, value, ts):
        ...
        r['_kalshi_final_target']=dict(self.kalshi_final_target) if self.kalshi_final_target else None
    ...
    def brti_websocket_worker(self):
        ...
        while self.running:
            ...
            try:
                ...
                ws.send(json.dumps({'id':1,'cmd':'subscribe','params':{'channels':['cfbenchmarks_value_5hz','cfbenchmarks_value'],'index_ids':['BRTI']}}))
                self.ws_status='LIVE'; self.ws_error=None
                while self.running:
                    raw=ws.recv(); msg=json.loads(raw)
                    typ=msg.get('type'); m=msg.get('msg') or {}
                    if typ=='cfbenchmarks_value':
                        if m.get('index_id')!='BRTI': continue
                        ft=m.get('last_60s_windowed_average_15min')
                        if isinstance(ft,dict) and ft.get('value') is not None:
                            try:
                                state={'schema':'KALSHI_CF_BENCHMARKS_FINAL_60S_V1','source':'KALSHI_CF_BENCHMARKS_WS',
                                       'index_id':'BRTI','value':float(ft.get('value')),'window_size':int(ft.get('window_size') or 0),
                                       'window_start_ts_ms':ft.get('window_start_ts_ms'),'window_end_ts_exclusive':ft.get('window_end_ts_exclusive'),
                                       'received_at':m.get('received_at'),'seq':msg.get('seq'),'sid':msg.get('sid')}
                                with self.lock:
                                    self.kalshi_final_target=state; self.kalshi_final_target_last_mono=time.monotonic()
                            except Exception: pass
                        continue
                    if typ!='cfbenchmarks_value_5hz': continue
                    if m.get('index_id')!='BRTI': continue
    ...
    def worker(self):
        # V2.28.45: market discovery/target publication is a retryable startup
        # state, not a fatal engine initialization condition.  Kalshi can expose
        # the deterministic ticker before the strike is populated, or briefly lag
        # all discovery routes at a quarter-hour handoff. Keep the engine alive.
        initialized=False
        while self.running and not initialized:
            try:
                ...
                msg=str(e)
                retryable=(
                    'TICKER_FOUND_TARGET_PENDING' in msg or
                    'WAITING_FOR_CURRENT_KXBTC15M' in msg or
                    'No current KXBTC15M contract resolved' in msg or
                    'No currently-open KXBTC15M contract found' in msg
                )
                if retryable:
                    self.waiting_for_next=True
                    self._set(error=msg)
                ...
                if not self.allow_coinbase_fallback:
                    self._set(error='Initialization failed: '+msg); return
                initialized=True
        if not self.running:
            return
...
def _research_row(r, now, orderflow, multi_exchange=None, multi_orderflow=None):
    ...
    tt=trend_transition_challenger_v2(r, None, now)
    row.update({
      'trend_transition_version':tt.get('version'),'trend_transition_state':tt.get('state'),'trend_transition_direction':tt.get('direction'),'trend_transition_confidence':tt.get('confidence'),'trend_transition_score':tt.get('score'),'trend_transition_armed':tt.get('armed'),'trend_transition_horizon_s':tt.get('horizon_s'),'trend_transition_authority':False,
...
def main():
    ...
    try:
        while True:
            ...
            if r is None:
                if err and ("No currently-open KXBTC15M contract found" in str(err) or "No current KXBTC15M contract resolved" in str(err) or "TICKER_FOUND_TARGET_PENDING" in str(err) or "WAITING_FOR_CURRENT_KXBTC15M" in str(err)):
