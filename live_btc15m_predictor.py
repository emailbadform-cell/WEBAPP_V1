...
def _mp_progress_state(r, mp):
    ...
    progress=max(0.0,min(1.0,raw_progress)); remaining=max(0.0,sign*(base-px))
...
RESEARCH_LOG_FILE = HERE / "BTC15M_V10_31_EXECUTION_EXHAUSTION_RESEARCH_LOG.csv"
...
class LiveEngine(LiveEngine):
    ...
    def _ensure_market(self):
        ...
        if remaining<=90 and self.next_market is None and time.monotonic()-self.last_market_discovery>=5.0:
