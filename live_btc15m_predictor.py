...
def trend_transition_challenger_v1(r, ticks=None, now=None):
    """Prospective-only 1m trend-transition challenger.

    Frozen after the 2026-09-19 historical discovery pass.  It never changes
    Decision, Settlement, GT, Trend, or Auto Paper.  The existing HH/HL/LH/LL
    trend remains the confirmation/control.  This runtime score intentionally
    uses 1m developing structure + MACD exhaustion + volatility context; 5m/15m
    are logged as context only and are not gates.
    """
    tr=int(r.get('m1_trend') or 0)
    if tr==0:
        return {'version':'TREND_TRANSITION_CHALLENGER_V1','state':'NEUTRAL / WAIT','direction':'NEUTRAL','confidence':0.0,'authority':False,'research_only':True,'horizon_s':65}
    # Opposite-direction pressure. Positive values mean the established 1m trend
    # is exhausting and the live candle is developing against it.
    mh=float(r.get('macd_hist') or 0.0); mv=float(r.get('macd_hist_velocity') or 0.0)
    body=float(r.get('body') or 0.0); rng=float(r.get('range5_abs') or 0.0)
    px=max(abs(float(r.get('btc') or 1.0)),1.0)
    # MACD exhaustion: histogram movement opposite the established trend.
    macd_exhaust=max(-2.0,min(2.0,(-tr*mv)/max(abs(mh),0.25)))
    # Developing 1m body opposite the established trend. body is normalized.
    structure=max(-2.0,min(2.0,-tr*body*2.0))
    # Volatility/context is supportive, never directional by itself.
    vol=max(0.0,min(1.0,(rng/px)*10000.0/12.0))
    raw=0.58*structure+0.34*macd_exhaust+0.08*vol
    conf=max(0.0,min(1.0,(raw+0.25)/1.50))
    desired='BEARISH' if tr>0 else 'BULLISH'
    armed=raw>=0.72
    state=(desired+' TRANSITION ARMED') if armed else (desired+' WATCH' if raw>=0.38 else 'CURRENT TREND HOLD')
    return {'version':'TREND_TRANSITION_CHALLENGER_V1','state':state,'direction':desired if raw>=0.38 else 'NEUTRAL','confidence':round(conf,4),'score':round(raw,4),'armed':bool(armed),'horizon_s':65,'m1_trend':tr,'m5_context':r.get('m5_trend'),'m15_context':r.get('m15_trend'),'macd_hist':mh,'macd_hist_velocity':mv,'developing_body':body,'authority':False,'research_only':True,'historical_reference':'NO_MOMENTUM_SELECTIVE_65S; 19/19 validation + 16/16 untouched; prospective confirmation required'}
...
def _research_row(r, now, orderflow, multi_exchange=None, multi_orderflow=None):
    ...
    tt=trend_transition_challenger_v1(r, None, now)
    row.update({
